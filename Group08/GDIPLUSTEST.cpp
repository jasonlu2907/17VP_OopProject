    ...
    case WM_PAINT:
        {
			...
			//vector<PointF> ar2 = { PointF(0,200), PointF(5,205) };
			...
			vector<char> buffer((istreambuf_iterator<char>(inPut)), istreambuf_iterator<char>()); //Doc tat ca file .svg
			...
			outPut << "<" << pNode->name() << " "; //ten cua node polyline / line / circle...
			for (rapidxml::xml_attribute<>* i = pNode->first_attribute(); i; i = i->next_attribute())
			{
				outPut << i->name() << "=\"" << i->value() << "\" "; //ten thuoc tinh stroke / width / height... va gia tri thuoc tinh
			}
			outPut << ">" << endl;
			if (pNode != nullptr) {
